...
{
    private readonly ISubscriptionService _subsciptionService;
    ...
    public NotificationProcessor(ISubscriptionService subsciptionService)
    {
    ...
    public void Process(AppleNotification notification)
    {
        ...
        if (!string.IsNullOrEmpty(v2Notification.DecodedPayload.Data.SignedTransactionInfo))
        {
    ...
    private static bool VerifyToken(string token)
    {
        try
        {
            ...
            var certeficatesItems = JsonConvert.DeserializeObject<IEnumerable<string>>(x5cCerteficates.ToString() ?? string.Empty);
